...
public class DiceBox_Skill : AbstractSkillSlot
{
    ...
    public override void ActivateSlotEffect() // spawns new dices up to its DiceAmoutEachTurn
    {
        ...
        {
            SpawnADie(SpecialDicePool[i], SpecialDicePool[i].DieSpecialNumberOrNull); // if the boon has a "Permanent number" difned, it will spawns with this number. otherwise "PermanentNumberOrNull" is defulted to be null
            diceLeftToSpawn--;
        }
